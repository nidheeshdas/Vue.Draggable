(function () {
    function buildVueDragFor(_, Sortable) {
        ...
        function computeIndexes(nodes) {
            return nodes.map(getRootFragment).filter(function (elt) {
                return !!elt;
            }).map(function (elt) {
                return (elt).scope.$index;
            }).value();
        }
            ...
            install: function (Vue) {
                ...
                mix(dragableForDirective, {
                    bind: function () {
                        ...
                        var indexes;
                        var userOnStart, userOnUpdate, userOnAdd, userOnRemove;
                        ...
                        userOnStart = options.onStart;
                        userOnUpdate = options.onUpdate;
                        userOnAdd = options.onAdd;
                        userOnRemove = options.onRemove;
                        options = _.merge(options, {
                            onStart: function (evt) {
                                indexes = computeIndexes(_.chain(evt.from.children));
                                console.log(indexes);
                                if (ctx.vm[userOnStart] && typeof ctx.vm[userOnStart] === 'function') {
                                    ctx.vm[userOnStart](evt);
                                }
                            },
                            onUpdate: function (evt) {
                                ...
                                if (ctx.vm[userOnUpdate] && typeof ctx.vm[userOnUpdate] === 'function') {
                                    ctx.vm[userOnUpdate](evt);
                                }
                            },
                            onAdd: function (evt) {
                                if (!!ctx.collection) {
                                    var addElement = getVmObject(evt.item);
                                    var localIndexes = computeIndexes(_.chain(evt.to.children).filter(function (elt) {
                                        return elt !== evt.item;
                                    }));
                                    ...
                                    removeNode(evt.item);
                                    insertNodeAt(evt.from, evt.item, evt.oldIndex)
                                    if (ctx.vm[userOnAdd] && typeof ctx.vm[userOnAdd] === 'function') {
                                        ctx.vm[userOnAdd](evt, addElement);
                                    }
                                }
                            },
                            onRemove: function (evt) {
                                ...
                                if (ctx.vm[userOnRemove] && typeof ctx.vm[userOnRemove] === 'function') {
                                    ctx.vm[userOnRemove](evt);
                                }
                            }
                        });
    ...
    if (typeof exports == "object") {
        ...
        define(['lodash', 'Sortable'], function (_, Sortable) {
            return buildVueDragFor(_, Sortable);
        });
